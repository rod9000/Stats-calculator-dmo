...
ELEMENT_MAP = {
    ...
    'Vacina': 'vaccine_skill_damage_percent',
}
...
for i in range(2, len(conj)):
    ...
    for col_start, cond_type in [(1, 'own'), (3, 'lvl'), (5, 'trans'), (7, 'element')]:
        ...
        if stat and stat != 'nan':
            ...
            if cond_type == 'element':
                ...
                # For own/lvl/trans: val is the reward value, not the requirement
                # Use digimon count for own/trans requirements
                conds.append({'t': cond_type, 'v': 0, 're': {mapped_stat: int(val) if val == int(val) else val}})
    ...
    base_id = re.sub(r'[^a-zA-Z0-9]', '_', name).upper()[:20]
    group_id = base_id
    counter = 1
    while group_id in seen_ids:
        group_id = f"{base_id[:18]}_{counter}"
        counter += 1
    seen_ids.add(group_id)
    
    # Update conditions with proper requirement values
    digimon_count = len(digimons)
    for cond in conds:
        if cond['t'] == 'own':
            cond['v'] = digimon_count
        elif cond['t'] == 'trans':
            cond['v'] = digimon_count
        elif cond['t'] == 'lvl':
            # Use a reasonable default based on digimon count
            cond['v'] = digimon_count * 100  # Assume ~100 levels per digimon
